/**
 * GET /api/newsletter/unsubscribe?email=...&sig=...
 *
 * Marks the subscriber unsubscribed. Same email-bound HMAC pattern as
 * confirm — the unsubscribe link is included in the footer of every
 * marketing email.
 */
...
export const GET: APIRoute = async ({ url }) => {
  ...
  if (!email || !sig || !secret) {
    return htmlPage(
      ...
      "We couldn't read the unsubscribe link. Reply to any letter and we'll take you off the list ourselves.",
      400,
    );
  }
  ...
  if (!timingSafeEqual(sig, expected)) {
    return htmlPage(
      ...
      "We couldn't verify this link. Reply to any letter and we'll take you off the list ourselves.",
      400,
    );
  }
  ...
  if (rows.length > 0 && !rows[0]!.unsubscribed_at) {
    await db
  ...
  return htmlPage(
    "You've been removed.",
    "We're sorry to see you go. You'll receive nothing further from us. If you change your mind, you're always welcome back.",
    200,
  );
};
